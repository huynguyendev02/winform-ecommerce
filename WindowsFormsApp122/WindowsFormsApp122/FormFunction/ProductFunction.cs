...
namespace WindowsFormsApp122.FormFunction
{
    public class ProductFunction
    {
        ...
        public static int itemSoldQuantity()
        {
            using (var context = new DatabaseEC())
            {
                Console.WriteLine(DateTime.Now);
                return !context.order_items.Sum(o => o.quanlity).HasValue ? 0: (int)context.order_items.Sum(o => o.quanlity);
            }
        }
        public static int itemSoldToday()
        {
            using (var context = new DatabaseEC())
            {
                string date = (DateTime.Now).ToString().Split(' ')[0];
                DateTime temp = DateTime.Parse(date);
                var quantity = context.order_items.Where(o => o.created_at == temp).Sum(o => o.quanlity);
                return quantity.HasValue ? (int)quantity : 0;
            }
        }
        public static dynamic topSellingProduct()
        {
            using (var context = new DatabaseEC())
            {
                var allPId = context.products.Select(p => p.id).ToList<int>();
                int quantity = 0;
                int pIdReturn = 0;
                foreach(var pid in allPId)
                {
                    var temp  = context.order_items.Where(o => o.product_id == pid).Sum(o => o.quanlity);
                    if (!temp.HasValue)
                        continue;
                    if ((int)temp > quantity)
                    {
                        quantity = (int)temp;
                        pIdReturn = pid;
                    }
                ...
                if (quantity > 0)
                    return context.products.Find(pIdReturn);
                return "";
            }
        }
    }
