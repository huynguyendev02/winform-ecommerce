...
namespace WindowsFormsApp122.Controllers.ManagermentController
{
    internal class InvoicesController
    {
        ...
        public void InvoiceContent()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Controllers.DrawManagement.DrawInvoices item = new DrawInvoices(index, (index + 1).ToString(), "Xìteen", "Hello 2", "Hello 3", "Processing", "120");
                item.Draw(pnContent);
                item = new DrawInvoices(index, (index + 1).ToString(), "Xìteen", "Hello 2", "Hello 3", "Complete", "120");
                item.Draw(pnContent);
                index++;
            }
        }
